...
void area(void){
        ...
        do {
            ...
            /*Alieni*/
            for(i=0;i<nnemici;i++){
                if(A[i].cord.alive) cancellasprite(A[i].old_cord.y,A[i].old_cord.x,'A');
                pthread_mutex_lock(&malieni);
            ...
            /*Bullet giocatore*/
            for(i=0;i<30;i++){
            if(BG[i].cord.alive) cancellasprite(BG[i].old_cord.y,BG[i].old_cord.x,'B');
            pthread_mutex_lock(&mbulletg);
            ...
            /*Bullet nemici*/
            for(i=0;i<30;i++){
            if(BN[i].cord.alive) cancellasprite(BN[i].old_cord.y,BN[i].old_cord.x,'H');
            pthread_mutex_lock(&mbulletn);
